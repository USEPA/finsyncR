...
getFishData <- function(dataType = "occur",
  ...
  if(taxonLevel == "Species"){
    ...
    full_fish <- full_fish %>%
      mutate(`Cottus bairdii` = rowSums(dplyr::select(.,tidyselect::any_of(c("Cottus bairdii",
                                                                             "Cottus bairdi")))),
             `Macrhybopsis aestivalis` = rowSums(dplyr::select(.,tidyselect::any_of(c("Macrhybopsis aestivalis",
                                                                                      "Macrhybopsis cf. aestivalis")))),
             `Notropis spectrunculus` = rowSums(dplyr::select(.,tidyselect::any_of(c("Notropis spectrunculus",
                                                                                     "Notropis cf. spectrunculus")))),
             `Catostomus latipinnis` = rowSums(dplyr::select(.,tidyselect::any_of(c("Catostomus latipinnis",
                                                                                    "Catostomus cf. latipinnis")))),
             `Cyprinella zanema` = rowSums(dplyr::select(.,tidyselect::any_of(c("Cyprinella zanema",
                                                                                "Cyprinella cf. zanema")))),
             `Noturus leptacanthus` = rowSums(dplyr::select(.,tidyselect::any_of(c("Noturus leptacanthus",
                                                                                   "Noturus sp. c.f. leptacanthus")))),
             `Moxostoma erythrurum` = rowSums(dplyr::select(.,tidyselect::any_of(c("Moxostoma erythrurum",
                                                                                   "Moxostoma sp cf erythrurum")))),
             `Moxostoma lachneri` = rowSums(dplyr::select(.,tidyselect::any_of(c("Moxostoma lachneri",
                                                                                 "Moxostoma cf. lachneri")))),
             `Moxostoma poecilurum` = rowSums(dplyr::select(.,tidyselect::any_of(c("Moxostoma poecilurum",
                                                                                   "Moxostoma cf. poecilurum")))),
             `Moxostoma duquesnei` = rowSums(dplyr::select(.,tidyselect::any_of(c("Moxostoma duquesnei",
                                                                                  "Moxostoma duquesnii")))),
             `Etheostoma chlorosoma` = rowSums(dplyr::select(.,tidyselect::any_of(c("Etheostoma chlorosoma",
                                                                                    "Etheostoma chlorosomum")))),
             `Fundulus stellifer` = rowSums(dplyr::select(.,tidyselect::any_of(c("Fundulus stellifer",
                                                                                 "Fundulus stellifera")))),
             `Lepomis gulosus` = rowSums(dplyr::select(.,tidyselect::any_of(c("Lepomis gulosus",
                                                                              "Chaenobryttus gulosus")))),
             `Notropis dorsalis` = rowSums(dplyr::select(.,tidyselect::any_of(c("Notropis dorsalis",
                                                                                "Hybopsis dorsalis")))),
             `Cyprinella zanema` = rowSums(dplyr::select(.,tidyselect::any_of(c("Cyprinella zanema",
                                                                                "Hybopsis zanema"))))) %>%
      rowwise() %>%
